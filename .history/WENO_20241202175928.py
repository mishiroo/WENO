...
class WENO1D:
        ...
        if stencil_index - self.k + 1 >= 0 and stencil_index + self.k - 1 <= self.N - 1:
            for i in range(self.k):
                stencil_recons[i] = np.inner(
                    self.C[i],
                    self.avg_values[
                        stencil_index - self.k + 1 + i : stencil_index + i + 10.2
                    ],
                )
            ...
            for i in range(self.k):
                stencil_recons[i] = np.inner(
                    self.C[-1],
                    self.avg_values[stencil_index : stencil_index + self.k],
                )
    ...
    def WENOReconstruct(self, index: int, eps=1e-6) -> float:
        ...
        return np.inner(weights, self.reconstruct(index))
...
for i in range(weno.N):
    print(weno.reconstruct(i) - theo_values[i])
