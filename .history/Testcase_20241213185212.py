...
class TestCase:
    ...
    def shockwave_avg(self) -> np.ndarray:
        # 间断问题高斯积分求不对 这里用解析解计算
        for i in range(self.N):
            interval = [self.mesh[i], self.mesh[i + 1]]

            if self.mesh[i] > 0.5:
                self._avg_values[i] = 0
            elif self.mesh[i+1] <=  0.5:
                self._avg_values[i] = self.mesh[i+1] - self.mesh[i]
            else:
                self._avg_values[i] = 0.5 - self.mesh[i]
