...
class WENO1D:
    ...
    def __init__(self, mesh: np.ndarray, avg_values: np.ndarray, order=5) -> None:
        ...
        if order == 1:
            ...
            raise ValueError("order must be 1, 3 or 5,   9(测试五阶算法用)")  # 其他阶的系数论文没给，先不写
    ...
    def StencilReconstruct_all(self, index: int) -> np.ndarray:
        ...
        for i in range(self.N + 1):
            self.stencil_solution[i] = self.StencilReconstruct(i)
