...
# 1D dimension
class WENO1D:
    def __init__(self, mesh: List[float], avg_values: List[float], order=5) -> None:
        """初始化WENO类

        Args:
            mesh (List[float]): 网格剖分[lb,...ub]
            avg_values (List[float]): 网格剖分[lb,...ub]
            order (int, optional): WENO算法阶数. Defaults to 5.
        """
        assert len(mesh) == len(
            avg_values
        ), "mesh and avg_values must have the same length!"
        self.order = order
        self.lb = mesh[0]
        self.ub = mesh[-1]
