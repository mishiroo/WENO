...
class WENO1D:
    ...
    def StencilReconstruct_all(self) -> np.ndarray:
        ...
        for i in range(self.N + 1):
            self.stencil_solution[i] = np.inner(
                self.weights_d, self.StencilReconstruct(i)
            )
    ...
    def BoundaryTreatment(self, theo_values: np.ndarray) -> None:
        ...
        for i in range(self.k):
            # 前k个模板
            self.stencil_solution[i] = theo_values[i]
            # 后k个模板
            self.stencil_solution[self.N - i] = theo_values[self.N - i]
