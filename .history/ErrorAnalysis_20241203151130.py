...
def SaveError(fn: str, NList: List[int], L2ErrorList: List[float], LinfErrorList: List[float]) -> None:
    """保存误差结果
...
def Draw(mesh: np.ndarray, theo_values: np.ndarray, weno_values: np.ndarray, show=False, fn: str = None):
    """只绘制剖分为N的精确值与数值解对比图
    ...
    # 绘制理论解（精确解）
    plt.plot(
    mesh, theo_values, label="Exact Solution", color="blue", linestyle='-', linewidth=2, marker='o', markersize=5, markevery=10
)

    # 绘制WENO数值解
    plt.scatter(
    mesh, weno_values, label="WENO Solution", color="red", marker="D", s=80, edgecolor='black', linewidth=2
)
