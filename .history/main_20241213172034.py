...
for n in NList:
    ...
    # 用户只需要实现需要的函数即可
    theo_values = test_case.getTheoValues(TestCase.exp_function)
    avg_values = test_case.getAverageValues(TestCase.exp_function)

    weno = WENO.WENO1D(mesh=mesh.mesh, avg_values=mesh.avg_values, k=3)
