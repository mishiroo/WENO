...
def AccuracyOrders_h(hList: List[float], ErrorList: List[float]) -> np.ndarray:
    """对两组及以上剖分结果进行收敛阶分析 返回长度为len(hList)第一个元素为None的收敛阶数组
    输入参数是剖分长度
    Args:
        hList (List[int]): 网格剖分长度列表
        ErrorList (List[float]): 误差结果列表

    Returns:
        np.ndarray: 收敛阶列表
    """
    N = [int(1/h) for h in hList]
    return AccuracyOrders(N, ErrorList)
